...
def tiled_latent_inference(
    ...
    lq_mod_cond = lq_tensor if getattr(args, "use_lq_token_modulation", False) else None
        ...
        for step_i in range(n_steps):
            ...
            for hi in h_pos:
                for wi in w_pos:
                    ...
                    lq_mod_tile_cond = None
                    lq_mod_tile_weak = None
                    if lq_mod_cond is not None:
                        ph_s, pw_s = hi * AE_FACTOR, wi * AE_FACTOR
                        ph_e = min((hi + lt_size) * AE_FACTOR, lq_tensor.shape[2])
                        pw_e = min((wi + lt_size) * AE_FACTOR, lq_tensor.shape[3])
                        lq_mod_tile_cond = lq_tensor[:, :, ph_s:ph_e, pw_s:pw_e]
                        lq_mod_tile_weak = vosr_model.interpolate(
                            lq_mod_tile_cond,
                            torch.zeros_like(lq_mod_tile_cond),
                            weak_str,
                            vosr_model.interp_type,
                        )
                    ...
                    if use_cfg:
                        ...
                        model_lq_mod = None
                        if lq_mod_tile_cond is not None:
                            model_lq_mod = torch.cat([lq_mod_tile_cond, lq_mod_tile_weak], dim=0)
                        d_out = model(model_inp, model_t, z=model_z, lq_mod=model_lq_mod)
                        d_cond, d_weak = d_out.chunk(2)
                        u_tile = d_weak + cfg_scale * (d_cond - d_weak)
                    else:
                        u_tile = model(inp_cond, t_cur.expand(b), z=z_fea_tile, lq_mod=lq_mod_tile_cond)
