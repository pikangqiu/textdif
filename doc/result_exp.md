...
- **★ 1.4B 正确 shortcut 蒸馏已启动(2026-06-24,199 GPU2,3)—— 复刻 0.5B 2a+2d 配方**:
  ...
  - **⚠️ 训练在 step ~16400 被第三方 OOM 中断(2026-06-27)**:跑到 ~step16400(已存 ckpt-16000)时,**有个新租户进程(26GB)挤上共享 GPU4**(本就有 15GB 共租 + 我的 38GB)→ 超 80GB → 我的 rank3 `torch.OutOfMemoryError` 崩;随后 6 次自动重试又因 GPU1-4 都被别人占满连续失败 → runner 退出。**正是当初纳入共享 GPU4 凑 4 卡的风险兑现**。干净 ckpt 停在 **16000**(计划 20000 的 80%)。
  - **★★★ 1.4B band sweep 结果(2026-06-27,ckpt 12000-16000,两阶段:先全推理后 metric sweep)—— 1.4B 与 0.5B 文字指标打平、PSNR 更高、且已平台**:协议①(199 osediff OCR_CHARACC=1−CER + pyiqa)+协议②(本机 TESTR)。

    | ckpt | CharAcc | PSNR | det F1 | E2E None | E2E Full |
    |---|---|---|---|---|---|
    | 12000 | 68.91 | 25.50 | 78.52 | 41.80 | 54.73 |
    | 13000 | 68.80 | 25.51 | 78.16 | 41.58 | 53.67 |
    | 14000 | 68.84 | 25.52 | 78.65 | 40.76 | 52.63 |
    | 15000 | 68.31 | 25.53 | 78.76 | 40.91 | 52.85 |
    | 16000 | 68.62 | 25.54 | 78.76 | 40.64 | 52.63 |
    | **1.4B 均值** | **68.70** | **25.52** | **78.57** | **41.14** | **53.30** |
    | **0.5B 冠军带** | 68.74–69.02 | 24.67 | 78.32 | 42.59 | 52.71 |

    - **全 IQA 指标(协议①,同一 metric 脚本)1.4B 均值 vs 0.5B 2a+2d 带均值(16-19k)**:

      | 指标 | 1.4B(12-16k) | 0.5B 2a+2d(16-19k) | 谁优 |
      |---|---|---|---|
      | PSNR↑ | **25.52** | 24.58 | 1.4B +0.94 |
      | SSIM↑ | 0.8162 | **0.8311** | 0.5B +0.015 |
      | LPIPS↓ | 0.2388 | 0.2383 | ~平 |
      | DISTS↓ | **0.1949** | 0.2021 | 1.4B |
      | NIQE↓ | **6.96** | 9.55 | 1.4B 大优 |
      | MUSIQ↑ | 44.85 | **52.47** | 0.5B 大优 |
      | MANIQA↑ | 0.3135 | **0.3988** | 0.5B 大优 |
      | OCR_EM↑ | **0.6001** | 0.5926 | 1.4B |
      | CharAcc↑ | 0.6870 | 0.6885 | ~平 |
      | det F1↑ | 78.57 | 78.32 | ~平 |
      | E2E None↑ | 41.14 | 42.59 | 0.5B |
      | E2E Full↑ | 53.30 | 52.71 | ~平 |

    - **结论**:(1) **文字识别/检测全线打平**(CharAcc/EM/det/E2E)——**放大到 1.4B 没有提升文字指标**。(2) **典型「失真 vs 感知」分裂**:1.4B 在 PSNR↑/NIQE↓/DISTS↓(更忠实、少失真)赢,0.5B 在 MUSIQ/MANIQA/SSIM(无参考感知锐度)赢。(3) **⚠️ 重大 confound:1.4B=Qwen f8c16 VAE、0.5B=SD2 f8c4 VAE**——MUSIQ/MANIQA/NIQE 对 VAE 纹理风格极敏感,故这些差异**主要是 VAE 不同、非纯规模效应,不能干净归因到"1.4B"**(SD2 VAE 产更多高频纹理→无参考指标高;Qwen VAE 更平滑忠实→PSNR/NIQE 好)。(4) **12000-16000 全平台**(CharAcc 68.3-68.9、det 78.5-78.8、PSNR 25.50-25.54 几乎不动)→ ckpt-16000 完全有代表性,补到 20000 必然停在此带,无需续训。
    - **论文含义(支撑主线)**:1.4B 学生(68.70)同样**远超其 teacher(65.63)**,与 0.5B(teacher 63.78→学生 68.8)同构;但**文字精度受 OCR 监督上限封顶,不随模型容量提升**——规模只买到像素保真(PSNR),买不到文字准确率。**这正面支撑"贡献点在 OCR-aware loss 而非规模"的论文主张**。规模轴可作为"PSNR↑但文字饱和"的补充消融。
    - 资产:`sweep_1p4b_phased.sh`(本机编排,两阶段)、`infer_1p4b_worker.sh`/`metric_1p4b_worker.sh`(199)、`sweep_1p4b_results.log`。**决定:不续训到 20000(已平台);1.4B 收尾,结论=规模买 PSNR 不买文字精度。**

---

### ★ UTEP 统一 baseline 对比（`/data/ywk/eval` 框架，2026-06-28）—— 开源方法同管线自跑 + 对标 TAIR published
**目的**:回应"扩散时代如何公平对比"。开源 baseline(SeeSR/OSEDiff/DiffBIR-v2/FaithDiff)原版 git + 官方权重,**统一管线**重跑:输入同为 real_test/LQ(128)→×4→512(与我们模型 inference_vosr_onestep 逐字一致),评测 = TESTR + **ABCNet v2**(AdelaiDet,补 4 处兼容补丁,HQ 校准对上 TAIR Table2/3 ±1~3) 两 spotter + 协议① PP-OCRv5(同我们 68.74 口径)。环境/补丁/坑全记 `/data/ywk/eval/SETUP.md`。

**(a) Real-Text 847 · 两 spotter(det F1 / E2E None / Full)**
| 方法 | TESTR | ABCNet v2 |
|---|---|---|
| OSEDiff | 71.0/27.3/38.1 | 48.5/16.5/29.9 |
| SeeSR | 71.2/30.2/39.4 | 69.1/28.0/44.1 |
| DiffBIR(v2) | 65.8/22.3/32.8 | 57.9/20.8/38.1 |
| FaithDiff | 74.1/30.4/40.5 | 71.5/27.0/42.6 |
| **我们 2d** | 77.0/44.6/54.7 | (待补) |
| **我们 2a+2d** | 78.3/42.6/52.7 | (待补) |

**(b) Real-Text 847 · 协议① PP-OCR(PSNR/SSIM/LPIPS/EM/CharAcc)**
| 方法 | PSNR | SSIM | LPIPS | EM | CharAcc |
|---|---|---|---|---|---|
| OSEDiff | 24.45 | 0.808 | 0.240 | 50.08 | 55.78 |
| SeeSR | 23.97 | 0.790 | 0.276 | 48.64 | 54.53 |
| DiffBIR(v2) | 19.46 | 0.389 | 0.602 | 37.22 | 42.44 |
| FaithDiff | 24.59 | 0.772 | 0.255 | 51.36 | 57.70 |
| **我们 2d** | 25.08 | 0.839 | 0.243 | 58.97 | 68.28 |
| **我们 2a+2d** | 24.67 | 0.832 | 0.237 | 59.23 | 68.74 |
→ **同口径下我们 CharAcc 68 vs baseline 42–58、E2E 也领先,且 PSNR/SSIM/LPIPS 不输** → 双赢,非"牺牲保真换识别"。

**(c) 对标 TAIR published(Table 3)的判断 —— setup 正确,缺口是 published 系统性偏高**
- 输入对所有方法**同为 128 LQ**(TeReDiff val `lq_img_path=real_test/LQ` 也是 128,其 "512" 是内部 SwinIR cleaner 放大后的工作分辨率,非更优输入)。
- **检测端对齐**:我们 baseline det F1 全在 TAIR ±3~5 内(DiffBIR 因 v2 权重略低)。**HQ 校准两 spotter/两数据集都对上 TAIR(±1~3)** → 管线精确。
- **E2E 系统性低 ~10~16**,且**所有方法含 TeReDiff 自身**(官方权重复现 None 33.5 vs 论文 49.39,−16)都低 → 是 **TAIR published 普遍不可复现/偏高**,非我们 bug。**结论:自评表为准,TAIR published 作 as-reported(标"官方权重不可复现")**。

**(d) SA-Text 三级 · 两 spotter(det F1 / E2E None;无词表故 None;⚠️我方含训练污染)**
| 方法 | lv1 (TESTR·ABCNet) | lv2 | lv3 |
|---|---|---|---|
| OSEDiff | 61.5/23.2·65.3/21.8 | 57.5/21.2·62.3/18.0 | 48.2/13.7·51.7/12.4 |
| SeeSR | 63.5/23.1·65.2/20.8 | 60.9/20.3·63.1/18.7 | 50.9/14.4·50.6/12.4 |
| DiffBIR(v2) | 63.0/22.5·62.1/19.4 | 57.4/18.1·58.6/16.0 | 48.7/12.3·47.2/10.8 |
| FaithDiff | 67.2/24.3·69.2/21.0 | 64.5/21.8·66.4/19.3 | 53.9/14.7·55.2/12.0 |
| **我们 2a+2d(含污染)** | 68.9/32.0·69.7/29.1 | 65.0/29.3·66.7/28.2 | 57.9/22.8·58.6/21.7 |
→ baseline re-run ≈ TAIR Table2 published(可信度验证);我方 2a+2d 三级全面领先但**带训练污染脚注**,干净结论看 Real-Text 847。

**(e) TeReDiff 的 Real-CE 结果(PRISM Table1b,RealCE-val,PP-OCRv5,TeReDiff 重训 BTL)**
| | PSNR | LPIPS | FID | ACC | NED |
|---|---|---|---|---|---|
| TeReDiff ×2 | 17.81 | 0.291 | 88.15 | 52.27% | 0.697 |
| TeReDiff ×4 | **17.03** | 0.363 | 106.13 | **34.23%** | 0.568 |
→ **关键佐证**:TeReDiff(全图文本方法,与我们同类)Real-CE ×4 仅 PSNR 17.03/ACC 34%,**与我们 Real-CE(PSNR 17.8~18.0、CRNN acc ~0.30)同水平**。**全图文本方法在 Real-CE 普遍弱(line-level 法 54–65% 因任务更易)→ 我们 Real-CE 弱是全图任务共性,非缺陷**。

**(f) Real-CE 跨域 · 4 baseline 同协议(eval_realce.py,官方 CRNN,261图/3414区域)** —— vs 我们模型(同 CRNN 口径)
| 方法 | PSNR | SSIM | LPIPS | **rec_acc** | rec_NED |
|---|---|---|---|---|---|
| OSEDiff | 17.57 | 0.649 | 0.270 | 0.2329 | 0.529 |
| SeeSR | 17.76 | 0.636 | 0.290 | 0.2291 | 0.520 |
| DiffBIR(v2) | 15.76 | 0.246 | 0.579 | 0.1629 | 0.409 |
| FaithDiff | 17.94 | 0.598 | 0.286 | 0.2337 | 0.524 |
| **我们 2a+2d(零样本)** | 17.17 | 0.656 | 0.289 | **0.2973** | 0.616 |
| **我们 2a-tiles(微调)** | 17.79 | — | — | **0.3012** | — |
| RRDB(官方,Real-CE 从头训) | 20.14 | 0.721 | 0.203 | 0.3093 | 0.662 |
→ **同 CRNN 口径下我们 rec_acc 0.30 >> 4 个通用扩散 baseline 0.16–0.23**(DiffBIR-v2 仅 0.163 崩),**逼近"在 Real-CE 从头训"的 RRDB 0.309**。通用扩散 baseline 在 Real-CE 识别集体掉到 0.23 以下,印证"通用 SR 毁中文文字、我们补回"的跨域故事。结合 (e) TeReDiff(全图,PP-OCR)Real-CE 也仅 0.34 → **全图文本方法 Real-CE 普遍弱是任务共性**,而我们在同类里反而领先 baseline。
